  ...
  if (is_file) {
    ...
    if (file.Exists())
      ...
      error.SetErrorStringWithFormat(
          "súbor zadaný v možnosti --source (-s) neexistuje: '%s'",
          command.c_str());
  } else
    command_set->push_back(InitialCmdEntry(command, is_file));
}
...
SBError Driver::ProcessArgs(const opt::InputArgList &args, bool &exiting) {
  ...
  if (auto *arg = args.getLastArg(OPT_core)) {
    ...
    if (!file.Exists()) {
      error.SetErrorStringWithFormat(
          "súbor zadaný v možnosti --core (-c) neexistuje: '%s'",
          arg_value);
      return error;
    }
  ...
  if (auto *arg = args.getLastArg(OPT_file)) {
    ...
    if (file.Exists()) {
      ...
      error.SetErrorStringWithFormat(
          "súbor zadaný v možnosti --file (-f) neexistuje: '%s'",
          arg_value);
      return error;
    }
  }
  ...
  if (auto *arg = args.getLastArg(OPT_arch)) {
    ...
    if (!lldb::SBDebugger::SetDefaultArchitecture(arg_value)) {
      error.SetErrorStringWithFormat(
          "neplatná architektúra v možnosti -a alebo --arch: '%s'", arg_value);
      return error;
    }
  }
  ...
  if (auto *arg = args.getLastArg(OPT_attach_pid)) {
    ...
    if (remainder == arg_value || *remainder != '\0') {
      error.SetErrorStringWithFormat(
          "Nepodarilo sa konvertovať PID procesu: \"%s\" na pid.", arg_value);
      return error;
    }
  }
  ...
  if (m_option_data.m_process_name.empty() &&
    ...
    WithColor::warning() << "argumenty programu sú pri pripájaní ignorované.\n";
  }
...
int Driver::MainLoop() {
  ...
  if (!m_option_data.m_repl) {
    ...
    WithColor::warning() << "príkazy určené na spustenie po načítaní súboru (cez -o "
                            "alebo -s) sú v režime REPL ignorované.\n";
  }
...
static void printHelp(LLDBOptTable &table, llvm::StringRef tool_name) {
  std::string usage_str = tool_name.str() + " [možnosti]";
  table.printHelp(llvm::outs(), usage_str.c_str(), "LLDB", false);

  std::string examples = R"___(
PRÍKLADY:
  Debugger možno spustiť v niekoľkých režimoch.

  Odovzdanie spustiteľného súboru ako pozičného argumentu pripraví lldb na
  ladenie daného spustiteľného súboru. Na rozlíšenie medzi argumentmi odovzdanými
  lldb a argumentmi odovzdanými ladenému spustiteľnému súboru musia argumenty
  začínajúce na - byť odovzdané po --.

    lldb --arch x86_64 /path/to/program program argument -- --arch armv7

  Pre pohodlie je podporované aj odovzdanie spustiteľného súboru po --.

    lldb --arch x86_64 -- /path/to/program program argument --arch armv7

  Odovzdanie jednej z možností pripojenia spôsobí, že lldb sa okamžite pripojí
  k danému procesu.

    lldb -p <pid>
    lldb -n <process-name>

  Odovzdanie --repl spustí lldb v režime REPL.

    lldb -r

  Odovzdanie --core spôsobí, že lldb bude ladiť súbor core.

    lldb -c /path/to/core

  Možnosti príkazov možno kombinovať s týmito režimami a spôsobiť, že lldb
  spustí zadané príkazy pred udalosťami alebo po nich, napríklad pri načítaní
  súboru alebo páde, v poradí zadanom na príkazovom riadku.

    lldb -O 'settings set stop-disassembly-count 20' -o 'run' -o 'bt'
    lldb -S /source/before/file -s /source/after/file
    lldb -K /source/before/crash -k /source/after/crash

  Poznámka: V režime REPL sa nenačíta žiadny súbor, takže príkazy určené na
  spustenie po načítaní súboru (cez -o alebo -s) budú ignorované.)___";
  llvm::outs() << examples << '\n';
}
...
int main(int argc, char const *argv[]) {
  ...
  llvm::setBugReportMsg("PROSÍM odošlite hlásenie o chybe na " LLDB_BUG_REPORT_URL
                        " a priložte výpis zásobníka volania pri páde.\n");
#else
  llvm::setBugReportMsg("PROSÍM odošlite hlásenie o chybe na " LLDB_BUG_REPORT_URL
                        " a priložte správu o páde z "
  ...
  if (MissingArgCount || input_args.hasArg(OPT_UNKNOWN)) {
    llvm::errs() << "Použite '" << argv0
                 << " --help' pre úplný zoznam možností.\n";
    return 1;
  }
  ...
  // ako 1 sekunda.
  {
    ...
    if (future.wait_for(std::chrono::seconds(1)) == std::future_status::timeout)
      fprintf(stderr, "Čakanie na dokončenie úloh na pozadí...\n");
